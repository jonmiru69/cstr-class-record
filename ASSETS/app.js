(() => {
  ...
  // Grading-weight distribution (Written Works / Performance Tasks / Quarterly
  // Assessment), selectable per section via the Subject dropdown on each sheet.
  // Per current DepEd guidance every subject here uses the same 20-50-30 split
  // (and the same 30-30-40 QA intra-split), but each subject has its own entry
  // so a future distribution change only needs to be edited in ONE place below.
  ...
  function renderSectionRecord() {
    ...
    const activeSubject = state.sections[section.id].subject || DEFAULT_SUBJECT;
    ...
    const qaIntraLabel = weights.qaIntra.map((value) => `${Math.round(value * 100)}%`).join(" / ");

    return `<div class="record-section"><div class="record-back">${button("← Back to sections", "go-records")}</div>
      <div class="section-heading"><div>
        <div class="section-title-wrap"><h2>${escapeHtml(section.subject)}</h2>
        ...
        <div class="record-meta"><span><strong>${section.level}</strong></span><span>Weights: <strong>${weights.ww} / ${weights.pt} / ${weights.qa}</strong></span><span>Roster capacity: <strong>${section.rosterSize}</strong></span></div>
      </div></div>
      ...
      <div class="legend"><span><i class="dot dot-red"></i>Raw score above HPS - correct before finalizing</span><span><i class="dot dot-code"></i>A = Absent, counted as a zero against that item's HPS · E = Excused, L = Late, excluded from computation as if the activity never happened</span><span>QA slots use fixed ${qaIntraLabel} intra-weights</span></div>
  ...
  function renderRecordTable(section, period, weights) {
    ...
    const qaLabels = ["ST 1", "ST 2", "Term Exam"].map((label, index) => `${label} (${Math.round(weights.qaIntra[index] * 100)}%)`);
    ...
    return `<div class="table-wrap"><table class="record-table compact-record"><thead>
      <tr class="component-row"><th class="number-cell" scope="col" rowspan="3">#</th><th class="name-cell" scope="col" rowspan="3">Learner name</th><th class="component-header component-ww" scope="colgroup" colspan="13">Written Works (${weights.ww}%)</th><th class="component-header component-pt border-start-pt" scope="colgroup" colspan="11">Performance Tasks (${weights.pt}%)</th><th class="component-header component-qa border-start-qa" scope="colgroup" colspan="6">Quarterly Assessment (${weights.qa}%)</th><th class="initial-header" scope="col" rowspan="3">Initial<br>Grade</th></tr>
  ...
  app.addEventListener("change", (event) => {
    ...
    if (event.target.dataset.subjectSelect !== undefined) {
      ...
      render();
      setStatus("Subject changed — grading weights updated. Remember to Save Changes.");
    }
  });
