(() => {
  ...
  function componentExcelFormulas(startCol, length, excelRow, hpsRow, weight, isWeightedQa) {
    ...
    const valid = `SUMPRODUCT(--ISNUMBER(${scores}),--ISNUMBER(${hps}),--(${hps}>0))`;
    const raw = `SUMPRODUCT(N(${scores}),--ISNUMBER(${hps}),--(${hps}>0))`;
    const possible = `SUMPRODUCT(--ISNUMBER(${scores}),--ISNUMBER(${hps}),--(${hps}>0),${hps})`;
    ...
    if (isWeightedQa && length === 3) {
      ...
      const activeWeight = `SUMPRODUCT(${intraWeights},--ISNUMBER(${scores}),--ISNUMBER(${hps}),--(${hps}>0))`;
  ...
  function isLearnerAssessmentComplete(learner, period) {
    return ["ww", "pt", "qa"].every((kind) => learner[kind].every((score, index) => {
      ...
      const hps = Number(period[`${kind}Hps`][index]);
      ...
      return scorePresent && hpsPresent && Number.isFinite(raw) && Number.isFinite(hps) && hps > 0;
    }));
  }
