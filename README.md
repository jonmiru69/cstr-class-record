...
- Score cells accept a raw number or one of three attendance codes - **A** (Absent), **E** (Excused), **L** (Late) - which are excluded from the percentage math exactly like a blank slot, so they never get silently scored as zero.
...
- A score cell containing `A`, `E`, or `L` (case-insensitive; typed lowercase is auto-uppercased) is treated as excluded from that component's ratio, the same as a blank slot, via `isAttendanceCode`. It is never averaged in as a zero.
