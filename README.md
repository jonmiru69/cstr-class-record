...
- Live, cell-by-cell grade computation: every Initial Grade, percentage, and weighted score recalculates instantly as scores are typed, even while the roster is only partially filled.
- Score cells accept a raw number or one of four attendance codes: **A** (Absent) and **M** (Missing - present, no excuse) automatically score as zero against that item's HPS, so the activity still counts against the learner's cumulative total; **E** (Excused) and **L** (Late) are excluded entirely from the percentage math, exactly like a blank slot, so they never unfairly penalize the learner's grade. The **M** code is shaded red in the sheet to flag it at a glance.
- Spreadsheet-style bulk paste: click any Learner name cell or score cell, then paste a multi-row/multi-column block copied from Excel or Google Sheets. It fills the matching cells starting from the cell you clicked, correctly aligned across Name, Written Works, Performance Tasks, and Quarterly Assessment columns.
- Section tabs are color-coded per subject/section (green = Saint John Stone, blue = Saint Pedro Calungsod, red = Saint Ezekiel Moreno, purple = Saint Alfonso de Orozco) and arranged vertically for faster selection.
- A sticky header in the school's maroon-and-gold colors, carrying the CSTR crest, that stays pinned at the top of the page while the rest of the page scrolls.
- A Save Changes button grouped with Settings and Log out in the header, which writes the complete state - including roster, scores, periods, and compressed teacher photo - to one GitHub Gist JSON file.
...
- A score cell containing `A`, `E`, `L`, or `M` (case-insensitive; typed lowercase is auto-uppercased) is recognized via `isAttendanceCode`. `A` and `M` (`isZeroScoreCode`) score as zero against that item's HPS - the HPS still counts toward the component's denominator, so the activity is not simply dropped. `E` and `L` (`isExcludedCode`) are excluded from that component's ratio entirely, the same as a blank slot, so they never drag down the grade.
- The Excel export's live formulas mirror this same split: cells coded `A` or `M` contribute their HPS to the denominator (with a zero numerator), while `E`, `L`, and blanks are excluded, so recalculating the exported sheet in Excel matches the website.
...
- Section tab colors: Saint Alfonso de Orozco (Grade 8) = purple, Saint John Stone (Grade 8) = green, Saint Pedro Calungsod (Grade 8) = blue, Saint Ezekiel Moreno (Grade 9) = red. SHS records (Grade 11) remain pastel blue.
...
- **Open item to confirm:** the feature list above documents "JHS weights of 30 / 40 / 30 and SHS weights of 20 / 50 / 30," but every section's `weights` field in `assets/js/app.js` is currently `[20, 50, 30]` - the SHS weighting - including the four JHS sections (Grade 8 x3, Grade 9). This mismatch predates this round of revisions and was left unchanged since it affects live grade computation and should only be corrected once the intended JHS weighting for CSTR is confirmed.
